...
uint8_t read_ctl_code(char* ctlcode)
{
  ...
  {
    ctlcode_len++;
    serial_port.readBytes(&ctlcode[ctlcode_len], 1);
  }
  ctlcode_len++;
...
void parse_single_m_code(char* ctlcode, uint8_t cmdstart, uint8_t cmdend, uint8_t& xtermcolor)
{
...
void parse_m_code(char* ctlcode, uint8_t ctlcode_len)
{
  uint8_t i = 0;
  uint8_t lasti = 0;
  ...
  while (i < ctlcode_len)
  {
    ...
    parse_single_m_code(ctlcode, lasti, i, setxtermcolor); //[lasti, i)
    i++;
    lasti = i;
  }
}
...
void parse_ctl_code()
{
  ...
  if (ctl_type != 'm')
  {
    ...
    switch (ctl_type)
    {
      ...
      case 'r': //set scrolling range
        ...
        //tft.setCursor(0,0);
        break;
      case 'P':  //need to be implemented for vt220!!!!
        /*
  ...
  {
    parse_m_code(ctlcode, ctlcode_len);
  }
}
